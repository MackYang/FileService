...
namespace System
{
    public static class StringExtension
    {
        ...
        /// <summary>
        /// 为字体添加指定颜色,用于HTML展示页面中.
        /// </summary>
        /// <param name="s"></param>
        /// <param name="color"></param>
        /// <returns></returns>
        static public string AddColorForHTML(this string s, string color)
        {
        ...
        /// <param name="additionalStr">接在数字后的附加字符串,如%等</param>
        /// <param name="isStrict">是否是严谨的,如果为true时,数字后必须有附加字符时才添加颜色</param>
        /// <returns></returns>
        static public string AddHTMLColorForNum(this string s, string color, string additionalStr = "", bool isStrict = false)
        {
            Regex r = null;
            if (!string.IsNullOrWhiteSpace(additionalStr))
            {
                if (isStrict)
                {
                    r = new Regex(@"\d+,{0,1}\d+\.{0,1}\d*" + additionalStr + @"+|\d+" + additionalStr + "+");
                }
                else
                {
                    r = new Regex(@"\d+,{0,1}\d+\.{0,1}\d*" + additionalStr + @"*|\d+" + additionalStr + "*");
            ...
            {
                r = new Regex(@"\d+,{0,1}\d+\.{0,1}\d*|\d+");
            }
            return r.Replace(s, new MatchEvaluator((x) => x.ToString().AddColorForHTML(color)));
        }
