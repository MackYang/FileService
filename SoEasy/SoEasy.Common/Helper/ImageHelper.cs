...
namespace SoEasy.Common
{
    ...
    {
        static Gif.GifHelper gifHelper = new Gif.GifHelper();
        ...
        public static Image Base64ToImage(string base64String, OPResult opRes, bool throwException = false)
        {
            return ExceptionHelper.ExceptionRecord(() =>
            {
        ...
        public static bool CreateThumbnail(string sourceFile, string destFile, int width, int height, OPResult opRes, bool throwException = false)
        {
            ...
            return ExceptionHelper.ExceptionRecord(() =>
            {
                ...
                try
                {
                    switch (extensionName)
                    {
                        ...
                        case ".gif":
                            gifHelper.Thumbnail(sourceFile, width, destFile);
                            return true;
                        default:
        ...
        {
            string exName = FileHelper.GetExName(fileName);
        ...
        public static bool AddWatermarkText(string srcFile, string destFile, int fontSize, string text, OPResult opRes, bool throwException = false)
        {
            return ExceptionHelper.ExceptionRecord(() =>
            {
                ...
                if (Path.GetExtension(srcFile).ToLower() == ".gif")
                {
                    gifHelper.WaterMark(srcFile, text, destFile);
                    return true;
                }
                else
                {
        ...
        public static string CreateVerificationImage(string verificationText, int width, int height, OPResult opRes)
        {
            ...
            return ImageToBase64(_bitmap, ImageFormat.Jpeg, opRes);
        }
