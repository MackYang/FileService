...
namespace SoEasy.Common
{
    ...
    public class NetHelper
    {
        #region 发送前检查事件
        /// <summary>
        /// 可定义符合此委托的方法传递给OnCheckMail事件,在邮件发送前,被传递的方法将被调用,以检查是否能发送该邮件,仅当返回结果的State为Success时才发送
        /// </summary>
        /// <param name="mailInfo">邮件信息实体</param>
        public delegate OPResult CheckMail(MailInfo mailInfo, OPResult opRes);
        /// <summary>
        /// 检查能否发送邮件的事件,如果订阅,将在邮件发送前被调用
        /// </summary>
        static public event CheckMail OnCheckMail;
        /// <summary>
        /// 可定义符合此委托的方法传递给OnCheckSMS事件,在邮件发送前,被传递的方法将被调用,以检查是否能发送该邮件,仅当返回结果的State为Success时才发送
        /// </summary>
        /// <param name="smsInfo">短信信息实体</param>
        public delegate OPResult CheckSMS(SMSInfo smsInfo, OPResult opRes);
        /// <summary>
        ...
        public static string GetClientIP(OPResult opRes, bool throwException = false)
        {
        ...
        ///<param name="opRes">传入的操作结果状态必须为Success才会往下执行</param>
        public static void AsyncSendEmail(MailInfo mi, OPResult opRes)
        {
        ...
        ///<param name="smsInfo">短信信息实体</param>
        ///<param name="opRes">传入的操作结果状态必须为Success才会往下执行</param>
        public static void AsyncSendSMS(SMSInfo smsInfo, OPResult opRes)
        {
            if (opRes != null && opRes.State == Enums.OPState.Success)
            {
                opRes.State = Enums.OPState.Fail;
        ...
        public static IPInfo QueryIPInfoIP138(string ip, OPResult opRes, bool throwException = false)
        {
            return ExceptionHelper.ExceptionRecord(() =>
            {
                try
        ...
        /// <param name="throwException">出现异常时是否抛出</param>
        /// <returns>IP信息实体</returns>
        public static IPInfo QueryIPInfoBaiDu(string ip, OPResult opRes, bool throwException = false)
        {
            return ExceptionHelper.ExceptionRecord(() =>
            {
                try
        ...
        /// <returns></returns>
        public static OPResult Request(string url, Dictionary<string, object> paraDic, Enums.RequestType requestType, WebHeaderCollection requestHeaderCollection = null)
        {
            ...
            try
            {
                ...
                response.Close();
                opRes = JsonHelper.FromJsonString<OPResult>(str);
                return opRes;
            }
            catch (Exception ex)
            {
        ...
        /// <param name="requestType">请求方式</param>
        /// <returns></returns>
        public static string RequestNew(string url, Dictionary<string, object> paraDic, Enums.RequestType requestType, string contentType = "application/x-www-form-urlencoded;charset=GBK;", string accept = "", WebHeaderCollection requestHeaderCollection = null)
        {
            try
            {
                Uri uri = new Uri(url);
            ...
            {
                Utility.Logger.Error("发送HTTP请求时发生异常：" + ex.Message);
                return "";
            }
        }
    }
